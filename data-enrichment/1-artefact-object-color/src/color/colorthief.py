...
"""
    ...
    Adjustet by Soeren Raeuchle@3pc.de | Original: https://github.com/fengsp/color-thief-py/blob/master/colorthief.py
"""
__version__ = '0.2.1'
...
class ColorThief(object):
    ...
    def __init__(self, file: Union[str, Path, Image.Image], color_name_finder: ColorRepository = ColorRepository()):
    ...
    def get_color(self, quality=10):
        ...
        return list(palette.keys())[0]

    def get_color_palette(self, color_count=10, quality=10) -> List[Color]:
        ...
        """
        palette, pixles = self.get_palette(color_count, quality)
        colors = [Color(rgb=color, count=pixel_count, ratio=round((pixel_count / pixles) * 100, 2),
                        name=self._name_finder.nearest_color_name(color)) for color, pixel_count in palette.items()]
        return [color for color in colors if color.ratio > 0.0]

    def get_palette(self, color_count=10, quality=10) -> Tuple[Dict[Tuple[int, int, int], int], int]:
        ...
        numpy_array = arr.reshape(-1, 4)
        # select nth pixel to reduce quality
        reduced = numpy_array[::quality]
        # filter pixels by alpha value
        valid = reduced[reduced[:, 3] >= 220]
        valid = np.take(valid, [0, 1, 2], axis=1)
        pixels = list(map(tuple, valid))
        ...
        return cmap.palette, len(pixels)
...
class CMap(object):
    ...
    @property
    def palette(self):
        return dict((x, y) for x, y in self.vboxes.map(lambda x: (x['color'], x['vbox'].count)))
